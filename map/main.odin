...
main :: proc() {
	fmt.printf("Hellope! Let's deep dive into HashMap of Odin\n")
	fmt.printf("Map are a data type embedded directly to Odin language-level.\n")
	fmt.printf(
		"With nice and simple syntaxes: `map[K]V`, which map is keyword, K is key data type, V is value data type.\n",
	)
	...
	fmt.println()
	fmt.printf(
		"One other good thing of Odin Map, is that it supporting syntax to create map like JS. (Require #+feature dynamic-literals)\n",
	)
	...
	fmt.printf(
		"Let do some checking when work with map. \"A\" in str_dict = %v\n",
		"A" in str_dict,
	)

	fmt.println()
	fmt.printf(
		"For the case HashMap of Bitsquid Foundation only supports hash value as key for map.\n",
	)
	fmt.printf(
		"You can use map[u32]T or map[u64]T. But the algorithms underlying are unsure. So we move to next topic.\n",
	)
	...
	fmt.printf(
		"Maybe the struct look like: struct {{ len, cap: int, nexts: [^]int, key_value_pairs: [^]KV }}\n",
	)
	...
	fmt.printf(
		"Finally, we can conclusion that, Odin map are not the same with Bitsquid Foundation HashMap.\n",
	)
	fmt.printf(
		"If you truely want the exact Open Addressing Hash Map like Bitsquid Foundation, create a custom struct are enough.\n",
	)

	fmt.println()
	fmt.printf(
		"See code in hash_map/main.odin (you can double click to open file in some terminal)\n",
	)
}
